...
knowledge_repository = KnowledgeRepository(session_factory=lambda: build_session_factory(engine)())
...
def create_knowledge_entry(
    ...
    entry = service.create_entry(title=title_value, content=content_value)
    return KnowledgeEntryResponse.model_validate(entry)


@app.get("/knowledge", response_model=list[KnowledgeEntryResponse])
def list_knowledge_entries(service=Depends(get_knowledge_service)) -> list[KnowledgeEntryResponse]:
    entries = service.list_entries()
    return [KnowledgeEntryResponse.model_validate(entry) for entry in entries]


@app.get("/search", response_model=list[KnowledgeEntryResponse])
def search_knowledge(query: str, service=Depends(get_knowledge_service)) -> list[KnowledgeEntryResponse]:
...
@app.get("/summary")
def get_summary(text: str) -> dict[str, str]:
    return {"summary": ai_service.summarize(text)}


@app.get("/ocr")
def get_ocr_preview() -> dict[str, str]:
    return {"result": ocr_service.extract_text(b"sample")}
...
def get_ui_page() -> HTMLResponse:
    ...
    <head>
        ...
        <style>
            ...
            .card { background: #111827; padding: 1.5rem; border-radius: 12px; max-width: 720px; }
    ...
    <body>
        <div class=\"card\">
            ...
            <p>This is the lightweight browser UI for the personal knowledge workspace.</p>
            <ul>
                <li><a href=\"/health\">Health</a></li>
                <li><a href=\"/knowledge\">Knowledge entries</a></li>
                <li><a href=\"/search?query=fastapi\">Search examples</a></li>
            </ul>
            <p>You can also call the API endpoints directly from the browser or a client like Postman.</p>
        </div>
    </body>
    </html>
    """
